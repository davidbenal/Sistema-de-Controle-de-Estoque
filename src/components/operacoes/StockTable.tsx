...
export function StockTable({
  ...
  const renderIngredientRow = (ingredient: IngredientWithStockInfo) => (
    <tr key={ingredient.id} className="border-b last:border-0 hover:bg-gray-50">
      <td className="px-3 py-3 font-medium text-gray-900">{ingredient.name}</td>
      ...
      <td className="px-3 py-3 text-gray-600">{ingredient.unit || '-'}</td>
      <td className="px-3 py-3 text-gray-600">{ingredient.supplier_name || '-'}</td>
      <td className="px-3 py-3 text-gray-600">{(ingredient.min_stock || 0).toFixed(2)}</td>
      <td className="px-3 py-3 text-gray-600">{formatDate(ingredient.last_order_date)}</td>
      <td className="px-3 py-3">{renderReorderDate(ingredient.recommended_reorder_date)}</td>
      <td className="px-3 py-3">
        <DropdownMenu>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b">
            <tr>
              <th className="px-3 py-3 text-left font-medium text-gray-700">Ingrediente</th>
              <th className="px-3 py-3 text-left font-medium text-gray-700">Categoria</th>
              <th className="px-3 py-3 text-left font-medium text-gray-700">Estoque Atual</th>
              <th className="px-3 py-3 text-left font-medium text-gray-700">Unidade</th>
              <th className="px-3 py-3 text-left font-medium text-gray-700">Fornecedor</th>
              <th className="px-3 py-3 text-left font-medium text-gray-700">Mínimo</th>
              <th className="px-3 py-3 text-left font-medium text-gray-700">Último Pedido</th>
              <th className="px-3 py-3 text-left font-medium text-gray-700">Próximo Pedido</th>
              <th className="px-3 py-3 text-center font-medium text-gray-700 w-12">Ações</th>
            </tr>
          </thead>
  ...
  return (
    <div className="space-y-4">
      <Tabs defaultValue={centers[0]} className="w-full">
        <TabsList className="grid w-full" style={{ gridTemplateColumns: `repeat(${centers.length}, minmax(0, 1fr))` }}>
          {centers.map(center => {
            ...
            const centerLabels: Record<string, string> = {
              ...
              'congelado': 'Congelado',
              'sem-centro': 'Sem Centro',
            };

            return (
              <TabsTrigger key={center} value={center} className="relative">
                {centerLabels[center] || center}
          ...
          return (
            <TabsContent key={center} value={center} className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3 bg-gray-50 p-4 rounded-lg border">
                ...
                <div className="text-center">
                  <p className="text-xs text-gray-600 mb-1">OK</p>
